...
if __name__ == "__main__":
    #print(Crawl_PagePosts('https://www.facebook.com/Statebuilding.tw/'))
    pageurl = 'https://www.facebook.com/Statebuilding.tw/'
    # init parameters
    headers = {'accept': 'text/html',
               'sec-fetch-user': '?1'}
    rs = requests.Session()
    ...
    for content in soup.findAll('div', {'class':'userContentWrapper'}):
        ...
        print(content.find('img')['aria-label']) # publisher_name
        print(content.find('div', {'data-testid':'story-subtitle'})['id']) # publisher_id
        print(content.find('abbr')['data-utime']) #publish_time
        print(message) # message
        print(content.find('a')['href'].split('?')[0]) #link
    for mod in data['jsmods']['pre_display_requires']:
        try:
            feedback_df.append([
                mod[3][1]['__bbox']['result']['data']['feedback']['subscription_target_id'], # post id
